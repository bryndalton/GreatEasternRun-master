$(document).ready(function () {
    ...
    function FAQfilter(inp) {
        ...
        $('.LWfaqs .LWfaqsInner .FAQLandingPageRight .faqCategory').each(function () {
            ...
            if ($(this).children('.faqObject.notFiltered').length > 0) {
                ...
                id = $(this).attr('id');
                //console.log(id);
                $('.LWfaqs .LWfaqsInner .FAQLandingPageLeft .FAQLandingPageScroll .listItem[data-link="#' + id + '"]').addClass('notFiltered');
            } else {
                $(this).removeClass('notFiltered');
                id = $(this).attr('id');
                //console.log(id);
                $('.LWfaqs .LWfaqsInner .FAQLandingPageLeft .FAQLandingPageScroll .listItem[data-link="#' + id + '"]').removeClass('notFiltered');
            }
    ...
    function ridiculousFAQScroll() {
        ...
        var f = $('.LWfaqsInner').offset().top;
        var fh = $('.LWfaqsInner').outerHeight();
        //console.log(t + hh + 80);
    ...
    $('.LWfaqs .FAQsearchWrapper #FAQsearch').keyup(function () {
        var v = $(this).val();
        console.log(v);
        FAQfilter(v);
    });
    ...
    $('.LWfaqs .LWfaqsInner .FAQLandingPageRight .faqCategory .faqObject .LWfaqQuestion').on('click', function () {
        $(this).parent().toggleClass('open');
        $(this).siblings('.faqObjectAnswer').slideToggle();
    });

    $('.LWfaqs .LWfaqsInner .FAQLandingPageLeft .listItem').on('click', function () {
        var id = $(this).data('link');
        var elem = $('.LWfaqs .LWfaqsInner .FAQLandingPageRight .faqCategory' + id).offset();
        var t = elem.top;
